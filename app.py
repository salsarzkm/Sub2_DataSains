...
feature_order = model.input_features  # custom attribute, bukan feature_names_in_
...
with st.form("dropout_form"):
    ...
    input_data["Application_mode"] = st.selectbox("Metode Pendaftaran", list(range(1, 18)))
    input_data["Application_order"] = st.slider("Urutan Pilihan Jurusan (0=1st choice, 9=last)", 0, 9)
    input_data["Course"] = st.selectbox("Program Studi", list(range(1, 17)))
    ...
    input_data["Nacionality"] = st.selectbox("Kewarganegaraan", [1, 2, 6, 10, 12, 14, 17])
...
if submit:
    ...
    input_df = input_df[feature_order]  # urutkan kolom sesuai yang dipakai saat training
